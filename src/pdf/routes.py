...
def process_selection():
    ...
    # Crop the image
    try:
        cropped_image = pdf_processor.crop(
            x, y, width, height, canvas_width, canvas_height
        )
        ocr_reader = current_app.config["OCR_READER"]
        value = ocr_reader.extract_text(cropped_image)
